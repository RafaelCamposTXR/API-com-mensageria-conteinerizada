...
from schemas.users import UsuariosSchema
...
def inserir_usuario(usuario_data: UsuariosSchema):
    session = get_db()
...
def obter_usuarios() -> List[UsuariosSchema]:
    session = get_db()
    usuarios = session.query(Usuarios).all()
    session.close()
    return [UsuariosSchema(nome=usuario.nome, email=usuario.email, senha=usuario.senha) for usuario in usuarios]

# Função para atualizar um usuário existente
def atualizar_usuario(usuario_id: int, usuario_data: UsuariosSchema):
    session = get_db()
